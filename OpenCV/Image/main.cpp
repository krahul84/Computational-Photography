/// Implementation of Color2Gray Algorithms
...
int parse_args(int argc, char** argv)
{
    ...
    for (int i = 2; i < argc; i++)
    {
        ...
        {
            i++;
            printf("%s is invalid : color2gray file.ppm -theta 45 -alpha 10 -r 0 -q 0\n", argv[i]);
        }
...
int main(int argc, char* argv[])
{
    ...
    const char* fname = argc >= 2 ? argv[1] : "sign.ppm";
    if (PARSE_ARGS_AMY)
    ...
    printf("Implementing color2gray algorithm on \"%s\"\n with alpha=%f, theta=%f ",
   ...
   // Output Creation for Color2Gray and Color2Gray+Color Images with specified parameters
    if (r == 0)
